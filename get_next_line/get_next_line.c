...
/*   Updated: 2024/01/30 17:36:07 by ssibai           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	...
	2) cpy has no new line, so it copies to the buffer again
		...
		then frees buffer.
		b. if the buffer doesn't have a new line, it str joins everything
		 to cpy, then frees buffer.
*/
...
char	*get_the_line(char *cpy, int index)
{
	...
	if (!str)
		return (NULL);
	temp = gnl_ft_substr(cpy, index + 1, gnl_ft_strlen(cpy));
	while (i < index + 1)
	{
		str[i] = cpy[i];
		i++;
